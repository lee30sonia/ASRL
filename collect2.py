...
rat.makeAction(game, np.array([-game.get_game_variable(POSITION_X), -game.get_game_variable(POSITION_Y)]) / rat.nSteps)
simData = {'vpc': rat.vpc.w, 'mpc': rat.mpc.w, 'cpc': rat.cpc.obj.w, 'rw': rat.rw, 'mgc': [(s.alpha, s.beta, s.A, s.Wcpc) for s in rat.mgc.sheets]}
sio.savemat('data2.mat', simData, appendmat=False)
...
### plotting firing fields ###
#plot_receptive_field(rat.mgc.data.T, np.array(rat.traj), plot_size=[30,15], bin_length=int(len(rat.traj)/15), fig_size=[15,30], fname="mgc")
